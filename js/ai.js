	...
	for(var i=0;i < 3;i++){
		for(var j=0;j < 3;j++){
			...
			if(temp_board[i][j] == 0){
				...
				move.score = minimax(temp_board,player * -1).score;
				if(player == ai && move.score > ai_max){
					move.x = i;
					move.y = j;
					ai_max = move.score;
					//if(move.score == 10) break;
				}
				else if(player == human && move.score < human_min){
					move.x = i;
					move.y = j;
...
function register_move_one(board,k){
	...
	return function(){
		...
		if(is_win(board)) win.textContent = 'Player one wins...';
		else if(check_draw(board)) win.textContent = 'Game Draw...';
		
		ai_move = minimax(board,ai);
		board[ai_move.x][ai_move.y] = ai;
		this.textContent = handle_letter(human);
		if(is_win(board)) win.textContent = 'Player two wins...';
		else if(check_draw(board)) win.textContent = 'Game Draw...';
	}
}
